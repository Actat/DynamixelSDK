...
#define DEVICE_NAME \
  "/dev/ttyUSB0"  // [Linux]: "/dev/ttyUSB*", [Windows]: "COM*"
...
ReadWriteNode::ReadWriteNode() : Node("read_write_node") {
  ...
  set_position_subscriber_ = this->create_subscription<SetPosition>(
      ...
      [this](const SetPosition::SharedPtr msg) -> void {
        ...
        uint32_t goal_position =
            (unsigned int)msg->position;  // Convert int32 -> uint32

        // Write Goal Position (length : 4 bytes)
        // When writing 2 byte data to AX / MX(1.0), use write2ByteTxRx()
        // instead.
        dxl_comm_result = packetHandler->write4ByteTxRx(
            portHandler, (uint8_t)msg->id, ADDR_GOAL_POSITION, goal_position,
            &dxl_error);
        ...
        if (dxl_comm_result != COMM_SUCCESS) {
          ...
          /*
          RCLCPP_INFO(this->get_logger(), "Set [ID: %d] [%d]", msg->id,
                      msg->position);
                      */
        }
      });
  ...
  auto get_present_position =
    ...
    /*
    RCLCPP_INFO(this->get_logger(), "Get [ID: %d] [Present Position: %d]",
  ...
  get_position_server_ =
      create_service<GetPosition>("get_position", get_present_position);
}
...
void setupDynamixel(uint8_t dxl_id) {
  ...
  dxl_comm_result = packetHandler->write1ByteTxRx(
      portHandler, dxl_id, ADDR_OPERATING_MODE, 1, &dxl_error);
...
int main(int argc, char *argv[]) {
  ...
  packetHandler->write1ByteTxRx(portHandler, BROADCAST_ID, ADDR_TORQUE_ENABLE,
                                0, &dxl_error);
